...
4. Do **not** work more than one sub-issue.

## Completion signal

If, while implementing the feature, you notice all issues are closed, stop without committing anything and emit <promise>COMPLETE</promise>.
...
- **This is a layer in a stacked PR run.** The loop already created and checked out this layer's branch before you started, and it owns every stack operation.
- **Do not run any `git branch`, `git checkout -b`, `git push`, `gh pr create`, or `gh stack` command.** Commit only. The loop pushes the whole stack once your commit is in place.
- **Leave a clean working tree.** Commit everything you intend to keep — anything left uncommitted is carried into the next layer.
